...
export default function Cotizaciones() {
  ...
  return <section style={{padding:'1rem'}}><h2>Cotizaciones históricas</h2>
    ...
    {error ? <p role="alert">{error}</p> : datos===null ? <p>Cargando…</p> : !datos.length ? <p>No hay datos para esta fecha.</p> : <><p>Valor disponible al {fecha(hasta)}: <b>{num(datos[0].valor,4)}</b> · publicado el {fecha(datos[0].fecha)}</p><div className="ob-tablewrap"><table className="ob-table"><thead><tr><th>Fecha publicada</th><th className="ob-num">Valor</th></tr></thead><tbody>{datos.map(d=><tr key={d.fecha}><td>{fecha(d.fecha)}</td><td className="ob-num">{num(d.valor,4)}</td></tr>)}</tbody></table></div></>}
  </section>
}
